...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' SE-INSCREVE LÁ FX GUILHERME'))
})
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Adeus Amg... @${num.split('@')[0]}👋* \n_Não Vai Fazer Falta Mesmo kakaka Cuidado Ae... _`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
ramlan = body.slice(7)
reply('[❗] WAIT BRO GUE NULIS DUMLU YAKAN`) buff= await getBuffer(`https://api.vhtear.com/write?text=${ramlan}&apikey=${vhtear}`)
...
break
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return client.sendMessage(from, 'Onde está o código do idioma, mana : ${prefix}tts id ah yamate kudasai', text, {quoted: mek})
                ...
                var gh = body.slice(12)
                   ...
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
                     ...
                     reply('[❗] Paciência Novamente IG Está Carregando')
...
*/		 
				case 'welcome':
                  if (isBanned) return reply(ind.baned())				
					...
					if (Number(args[0]) === 1) {
						...
						reply('*⟪ Sucesso ⟫ Agora Quem Entra Neste Grupo Estarei Dando Boas Vindas*')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/group/welkom.json', JSON.stringify(welkom))
						reply('*⟪ Sucesso ⟫ Recurso De Boas Vinda Ao Grupo Desligado*')
					} else {
						reply(ind.satukos())
					}
					break
                 case 'event':
                  if (isBanned) return reply(ind.baned())                 
					...
					if (args.length < 1) return reply('Ekhemm >_<')
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*Evento Já Tá Ativo Pcr kakakaka*')
						event.push(from)
						fs.writeFileSync('./database/group/event.json', JSON.stringify(event))
						reply('*⟪ Sucesso ⟫ Ativa Evento No Grupo*')
					} else if (Number(args[0]) === 0) {
						event.splice(from, 1)
						fs.writeFileSync('./database/group/event.json', JSON.stringify(event))
						reply('*⟪ Sucesso ⟫ Ativa Evento No Grupo*')
				...
				case 'simih':
                  if (isBanned) return reply(ind.baned())				
					...
					if (Number(args[0]) === 1) {
						...
						samih.splice(from, 1)
						fs.writeFileSync('./database/group/simi.json', JSON.stringify(samih))
						reply('*⟪ Sucesso ⟫ Desligado O Recurso Simi Neste Grupo*')
					} else {
						reply(ind.satukos())
					}
					break
				case 'nsfw':
                  if (isBanned) return reply(ind.baned())				
					...
					if (Number(args[0]) === 1) {
						...
						reply('*⟪ Sucesso ⟫ Ativando o recurso nsfw no grupo*')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/group/nsfw.json', JSON.stringify(nsfw))
						reply('*⟪ SUKSES ⟫ Desligando o recurso nsfw no grupo *')
					} else {
						reply(ind.satukos())
					}
					break
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))				
					...
					no = 0
					for (Vamos avisa os administradores de grupo) {
						no += 1
                  ...
                  if (isBanned) return reply(ind.baned())					
					...
					if (args[0] === 'buka') {
						...
						client.groupSettingChange(from, GroupSettingChange.messageSend, falso)
					} else if (args[0] === 'tutup') {
						reply(`*Grupo Fechado Com Sucesso*`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, Verdade)
					}
					break
				case 'add':
                  if (isBanned) return reply(ind.baned())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))				
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args.length < 1) return reply('Deseja adicioná-lo, senhor? -_-')
					...
					try {
						...
						console.log('Error :', e)
						reply('Anjim que quiser adicionar em privado, tudo bem:)')
					}
					break
			     	case 'kick':
                  if (isBanned) return reply(ind.baned())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))			     	
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque O Alvo Que Você Quer Remove!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Mim Adicione Como Admin E Removerei Ele Tá Achando Que Sou Mágico Pra Ganha Admin Sozinho Krlho 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(O texto mencionado é verdadeiro)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Coloca Eu Como Admin Nessa Poha Vey Tá Achando Que Sou Mágico Pra Ganha Admin Sozinho@${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
                case 'hidetag':
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					client.sendMessage(De, opções de texto)
					Limite de espera add(sender)
					break
                case 'level':
                  if (isBanned) return reply(ind.baned())                
                if (!isRegistered) return reply(ind.noregis())
               ...
               client.sendMessage(from, resul, text, { quoted: mek})
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))                
				    ...
				    if (isLimit(sender)) return reply(ind.limitend(pusname))
				    if (!isBotGroupAdmins) return reply(ind.badmin())
				    linkgc = Espere Cliente.groupInviteCode (from)
           ...
           case 'setname':
                ...
                client.sendMessage(from, '⟪ Sucesso ⟫ Mude o nome do grupo', texto, {quoted: mek})
					break
                case 'setdesc':
                if (!isRegistered) return reply(ind.noregis())                
                if (!isGroup) return reply(ind.groupo())
                ...
                client.sendMessage(from, '⟪ SUKSES ⟫ Muda Descricão Do Grupo', text, {quoted: mek})
					break
           case 'demote':
                if (!isRegistered) return reply(ind.noregis())           
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('A marca alvo que você deseja Rebaixa!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
						...
						mentions(`Yahh @${mentioned[0].split('@')[0]} Seu título como Admin no grupo foi removido🏃`, mencionado, verdadeiro)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
                if (!isRegistered) return reply(ind.noregis())				
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque Alvo Que Você Deseja Promove!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Pago Um Guloso Pros Admin E Foi Promovido Hehe >_< :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Parabéns🥳 @${mentioned[0].split('@')[0]} *Pago Guloso Direitinho Pros Admin A Ganho Admin* >_<`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				case 'delete':
                if (!isRegistered) return reply(ind.noregis())				
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Carregando!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
...
*/
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply('URL NÃO ER VALIDO')
...
*/
				...
				if ( checkATMuser(sender) <= total) return reply(`desculpe mana, o dinheiro não é suficiente, colete o dinheiro primeiro> _ <não abra mano:v`)
            ...
            case 'bitly':
               ...
               client.updatePresence(from, Presence.composing) 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					teks = '♡───────────♡\n'
					for (Deixe i de resultado de dados) {
						teks += `*➸ Gambar* : ${i.image}\n*➸ Titulo* : ${i.title}\n*➸ link* : ${i.link}\n*➸ tag* : ${i.tag}\n♡───────────♡\n`
					}
					reply(teks.trim())
					await limitAdd(sender)
				...
				case 'mutual':
                  if (isBanned) return reply(ind.baned())
				if (!isRegistered) return reply(ind.noregiso9()
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                if (isGroup) return  reply( 'Não Pode Está Em Grupos')
...
case 'bc':
				...
				case 'setprefix':
					...
					if (!isOwner) return reply(ind.ownerb())
					prefix = args[0]
					reply(`*⟪ Sucesso Meu Chefe ⟫* Prefix Alterado ➸ : ${prefix}`)
					break
				case 'clearall':
					if (!isOwner) return reply(ind.ownerb())
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(Pedido Recebido, Bloquear ${body.slice(7)}@c.us`, Bloqueado)
					break
                    case 'unblock':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				    client.blockUser (`${body.slice(9)}@c.us`, "remove")
					client.sendMessage(Pedido Recebido, Desbloquear${body.slice(9)}@c.us`, Desbloqueado)
					break
				case 'leave': 
				if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				await reply(from, 'Aku pamit ya :)').then(() => client.groupLeave(groupId))
					...
					reply('Dê-me o novo perfil do meu chefe')
					break
				case 'clone':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerg())
					if (args.length < 1) return reply(' *MARQUE O @ DO ALVO QUE DESEJA CLONA!!!* ')
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag`avk')
					...
					try {
						...
						mentions(`Foto De Perfil Alterado Para @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply(ind.stikga())
					}
					await limitAdd(sender)
                ...
                case 'ban':
					...
					reply(`Nomor wa.me/${bnnd} Foi Danido!`)
					break
				case 'unban':
					if (!isOwner) return reply(ind.ownerb())
					bnnd = body.slice(8)
					...
					fs.writeFileSync('./database/user/banned.json', JSON.stringify(ban))
					reply(`Nomor wa.me/${bnnd} Foi Desbanido!`)
					break									
/*
]=====> PREMIUM MENU <=====[
*/					
